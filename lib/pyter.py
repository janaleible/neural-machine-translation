...
def main():
    ...
    import math
    args = parse_args()
    ilines = [preprocess(x, args.lang) for x in codecs.open(args.input, 'r', 'utf-8').readlines()]
    rlines = [preprocess(x, args.lang) for x in codecs.open(args.ref, 'r', 'utf-8').readlines()]
    ...
    for lineno, (rline, iline) in enumerate(itertools.izip(ilines, rlines), start=1):
        if args.force_token_mode:
            ...
            rline, iline = split(rline, args.lang), split(iline, args.lang)
